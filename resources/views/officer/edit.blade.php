...
@section('content')
    <form action="{{ route('officer.update', ['person' => $person]) }}" method="post">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="name" name="name" class="w-full border rounded-md px-3 py-2 text-gray-700"
                value="{{ $person->name }}" required>
        </div>
        <div class="mb-4">
            <label for="designation" class="block text-white font-bold">Designation:</label>
            <input type="text" id="designation" name="designation"
                class="w-full border rounded-md px-3 py-2 text-gray-700" value="{{ $person->designation }}" required>
        </div>
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <input type="tel" id="phone" name="phone" pattern="[0-9]{10}"
                class="w-full border rounded-md px-3 py-2 text-gray-700" value="{{ $person->phone }}" required>
        </div>
        <div class="mt-4 pb-4">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="nid" name="nid" pattern="[0-9]{10}"
                class="w-full border rounded-md px-3 py-2 text-gray-700" value="{{ $person->nid }}">
        </div>
        <div class="mt-6">
            <button type="submit"
                class="bg-gray-700 text-white px-4 py-2 rounded-md hover:bg-white hover:text-gray-700">Update</button>
        </div>
        @if ($errors->any())
            <ul>
